...
The system SHALL expose `PATCH /api/history/{id}/bookmark` that toggles the `is_bookmarked` field for the given item and returns the updated `HistoryItem`.

#### Scenario: Toggle bookmark from false to true
- **WHEN** client sends `PATCH /api/history/{id}/bookmark` on an item with `is_bookmarked: false`
- **THEN** server flips `is_bookmarked` to `true` and returns `200` with updated `HistoryItem`

#### Scenario: Toggle bookmark from true to false
- **WHEN** client sends `PATCH /api/history/{id}/bookmark` on an item with `is_bookmarked: true`
- **THEN** server flips `is_bookmarked` to `false` and returns `200` with updated `HistoryItem`
