...
The system SHALL provide `app/(main)/history/page.tsx` that on mount calls `getHistory(deviceId, { page: 1, limit: 20 })` from `api.ts` and renders the result. While loading, a skeleton MUST be shown. On success, items MUST be rendered newest-first. The page MUST display an empty state when `items.length === 0`.
...
- **WHEN** the user taps a history item card
