# Spec: device-id-client

## Purpose

Defines the device identity utilities for the MathSnap frontend. Covers `lib/device-id.ts` for persistent UUID generation via `localStorage`, and the `hooks/useDeviceId.ts` React hook for SSR-safe access to the device ID.
