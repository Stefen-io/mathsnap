...
The backend SHALL provide a FastAPI application defined in `src/server/main.py` that initializes with CORS middleware and an eager-loaded pix2tex model via the lifespan context manager.

#### Scenario: Application starts successfully

- **WHEN** the Docker container starts and `uvicorn main:app` is invoked
